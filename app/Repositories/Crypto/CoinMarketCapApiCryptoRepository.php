...
class CoinMarketCapApiCryptoRepository implements CryptoRepository
{
    ...
    public function find(string $id, string $currency): Crypto
    {
        ...
        $logoResponse = Http::crypto()->get('v2/cryptocurrency/info', [
            'id' => $id,
        ])->json();

        $coin['logo'] = $logoResponse['data'][$id]['logo'];
    ...
    private function getLogos(array $data): array
    {
        $ids = collect($data)->pluck('id')->toArray();

        $logosResponse = Http::crypto()->get('v2/cryptocurrency/info', [
            'id' => implode(',', $ids),
        ])->json();

        return $logosResponse['data'];
    }
}
